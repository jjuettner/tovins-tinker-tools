-- Ensure pgcrypto exists and digest algo arg is typed as text.
-- Fixes: function digest(bytea, unknown) does not exist (42883)
...
begin
  ...
  v_hash := encode(digest(convert_to(v_token, 'utf8'), 'sha256'::text), 'hex');
...
begin
  ...
  v_hash := encode(digest(convert_to(trim(p_token), 'utf8'), 'sha256'::text), 'hex');
